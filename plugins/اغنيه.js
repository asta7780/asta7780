  ...
  try {
    ...
    if (command == 'فيديو') {
      ...
      try {
        ...
        const aa_2 = await conn.sendMessage(m.chat, {video: {url: mediaa.result}, fileName: `error.mp4`, caption: `*『𝐒𝐇𝐀𝐃𝐎𝐖 bot』\n انت مسؤل عن ذنوب اغانيك*`, thumbnail: mediaa.thumb, mimetype: 'video/mp4'}, {quoted: m});
        if (!aa_2) {
        throw new Error('*[❗] El primero metodo fallo, intentando otro...*');
       }
      } catch {
        ...
        const json = await res.json();
        await conn.sendFile(m.chat, json.result.video, 'error.mp4', `*『🐉┇𝐒𝐇𝐀𝐃𝐎𝐖 𝙱𝙾𝚃』\n انا لا اتحمل ذنوب اغانيك او ما تشاهده*`, m);
      }
    }
  } catch {
