...
${sortedLevel.slice(0, len).map(({ jid, level }, i) => `*${i + 1}.* ${participants.some(p => areJidsSameUser(jid, p.id)) ? `(${conn.getName(jid)}) wa.me/` : '@'}${jid.split`@`[0]} ➭ _*مستوى ${level}*_`).join`\n`}
`.trim()
...
handler.tags = ['econ']
handler.command = ['ممنوع', 'ممنوع', 'ممنوع'] 
